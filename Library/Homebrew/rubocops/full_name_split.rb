    ...
    module Homebrew
      ...
      class FullNameSplit < Base
        ...
        def basename_call?(node)
          ...
          return false unless node.method?(:fetch) && node.arguments.length == 1
        ...
        def split_call?(node, split_call)
          return false unless split_call.method?(:split) && split_call.arguments.length == 1
          return false unless split_call.csend_type? == node.csend_type?
        ...
        def full_name_receiver?(receiver)
          return false if receiver.source.match?(%r{(?:\A|[.])tap(?:\.|&\.)full_name\z})
        ...
        def receiver_method_name(receiver)
          ...
          return unless argument.is_a?(RuboCop::AST::Node) && argument.str_type?
